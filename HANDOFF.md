...
- **2026-05-11** — Batch 15 confirmed as the next batch number (B11-B14 already used). New tracking spreadsheet `tallow-cream/tracking/Ad_Test_Variables_Tracker.csv` created to document what each ad in each batch tests (hook format, discovery character, mechanism delivery format, habit cessation type, husband notice format, identity payoff, setting). Retroactive fill for B1-B14, prospective design for B15. Purpose: when an ad wins, we know which specific variables were under test so we can iterate on those in the next batch.
- **2026-05-11** — B15 test design locked. Three ads test distinct variable combinations: Ad 1 Sister Visit (authority-referenced mechanism + family witness + light-avoidance cessation + specific verbal husband notice + recognition payoff), Ad 2 Group Text (peer self-investigation mechanism + cohort witness + photo-avoidance cessation + cumulative husband notice + visible-presence payoff), Ad 3 Old Notebook (heritage + retroactive science mechanism + generational artifact witness + time-overinvestment cessation + adjacent husband notice + inheritance payoff). Controls locked: Invisible Woman avatar, 10-element skeleton, voice rules, no em-dashes, comparison vs water-based moisturizers (not vs other tallow), $49.99 + 60-day guarantee.

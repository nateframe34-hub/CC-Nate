...
| Batch 12 testing (Invisible Woman, locked-formula rebuilds) | QUEUED — start after B7C1+B9C3 stabilizes | Don't overlap with current scale + B9C3 add | Plan: NEW dedicated ad set with all 3 B12 ads (Hairdresser, Pond's nostalgia, Daughter Sent It) at $20-30/day. Same broad audience as converters ad set. CBO at ad-set level lets the 3 angles compete for budget within the batch. |
| Batch 13 testing (Photo Avoider sub-sub-avatar) | QUEUED — start after B12 ad set is stable | Different test variable than B12 (avatar fit, not formula fit) | Plan: NEW dedicated ad set with all 3 B13 ads (Camera Roll, Christmas Card, Behind The Camera) at $20-30/day. Same broad audience. Tests Photo Avoider sub-avatar against the validated Invisible Woman variants. |
| Campaign structure | One campaign, multiple ad sets | None | Ad Set A: Proven converters (B7C1 + B9C3 + later B5C1). Ad Set B: B12 batch (3 angles). Ad Set C: B13 batch (3 angles). Future Ad Set D: B14 (Retinol Burnout). Future Ad Set E: B11 (Working Hands Man — different avatar entirely). All ad sets identical broad audience; creative does the targeting per locked principles. |
...
- **2026-05-04** — Testing structure locked: ONE ad set per batch, each ad set holds all 3 angles for that batch. Proven converters stay in their own dedicated ad set (B7C1 + B9C3 + later B5C1). Test ad sets are isolated from converter ad set so unproven creative doesn't compete with proven for budget. All ad sets share identical broad audience (35-65, female, US/EN); creative does the targeting per locked principles.
- **2026-05-04** — Testing roadmap for B12/B13 sequenced AFTER current scale + B9C3 add stabilizes. Don't overlap multiple changes (scale to $30 + add B9C3 + test new batches = three simultaneous variables = no clean attribution). Phase 1: scale + B9C3 today. Phase 2: B12 ad set in 2-3 days once Phase 1 settles. Phase 3: B13 ad set 4-5 days after B12 launches (gives B12 some independent data before B13 layers in).

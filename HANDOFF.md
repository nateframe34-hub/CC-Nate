...
| Purchase campaign B7C1 | SPENDING PROPERLY — 1 sale at $18.15 CPA, 2.75x gross ROAS (single SKU) | Pixel reporting $0 conversion value despite registering 1 purchase — value attribution broken | $10/day is producing 1 sale/~1.5 days; not enough volume to test additional ads. Scaling decision in progress. Performance is profitable at current CPA (well below $31 single-SKU break-even). |
| Pixel value attribution | BROKEN — purchases register but conversion value = $0 | Likely Shopify-side pixel config issue or value parameter not passing | Diagnose before scaling: Events Manager → Test Events → complete a test purchase → confirm `value` parameter is set on the Purchase event. Without value attribution, Meta can't optimize toward higher-AOV customers (matters for BOGO 50% when it's implemented). |
| Scaling decision | UNDER REVIEW | Needs to balance "more data faster" against "fresh Portfolio surge risk" | Safe aggressive: 30%/day to ~$25-30 over 2-3 days. Faster: jump to $25-30 in one move. Aggressive: jump to $40 in one move (4x scaling, real risk of learning-phase reset). |

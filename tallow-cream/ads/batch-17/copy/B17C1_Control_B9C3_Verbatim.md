...
> **THIS IS B9C3 VERBATIM.** Do not change a word. Everything else in B17 (C2, C3, C4) is byte-identical to this ad except for a single inserted Urgency Anchor block between the "Fourteen things" inventory and the discovery scene. C1 has no anchor and reads exactly as B9C3 has read since April 2026. The 17% LPV-to-purchase rate is the bar.
...
- **Variable isolated in C1:** none. This is the control.
- **What this cell answers:** the protected-budget validation of B9C3's ~17% LPV-to-purchase rate. Every prior B9C3 data point came from CBO; this is the first time it runs in its own ad set with a guaranteed daily budget.
- **Ad-set config:** ABO, $12.50/day, fresh BM after warmup gates, single broad audience (35-65, female, US/EN), Purchase optimization, Advantage+ Placements, same image as C2/C3/C4.
- **Density:** 10/10 per B9C3 self-audit.
- **Compliance:** clean per B9C3 audit.
- **Word count:** ~1,500.
